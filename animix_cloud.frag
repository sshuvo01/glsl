...
float pattern( in vec2 p, out vec2 qq, out vec2 rr )
{
    ...
    vec2 r = vec2( fbm( p + sin(p.x + u_time)*q + vec2(0.440,0.390) ),
                   fbm( p + 5.456*q + vec2(0.410,-0.080) * u_time ) );
...
float pattern2( in vec2 p, out vec2 qq, out vec2 rr)
{
